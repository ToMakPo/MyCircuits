function Part (x, y, label, id, type, name, visible, opacity, draggable, parent) {
    var _x, _y, _id, _type, _name, _label, _svg, _asset, _element, _elmID, _visible, _opacity, _draggable, _nodes, _rotation, _parent, _axle, _center, _bounds, _boundsBox, _rotateHandle, _moveHandle
    ...
    function draw(self) {
        ...
        _center.moveTo(self)
        _axle = NodeBuilder(self, 'Axle', 0, 0)
            .setColor('#9d92ad')
            .setSnapRadius(-1)
            .setOpacity(1)
            //.setVisibility(true)
        ...
        _bounds = _svg.group()
            .attr('id', _elmID + '_bounds')

        var w = 50, h = 40
        _boundsBox = _bounds
            .attr('id', _elmID + '_boundsBox')
            .rect(0, 0)
            .center(_x, _y)
            .fill('transparent')
        ...
        _rotateHandle = _bounds.path(d)
            .attr('id', _elmID + '_handle-rotate')
            .move(_x - w / 2 - 5, _y - h / 2 - 5)
            .addClass('handle')
            .fill('#6BB1C77F')
        ...
        _moveHandle = _bounds.path(d)
            .attr('id', _elmID + '_handle-move')
            .move(_x + w / 2 - 2, _y - h / 2 - 8)
            .addClass('handle')
            .fill('#6BB1C77F')
            .stroke({color: '#666', width: 0.25})
    ...
    function setBounds(self, width, height) {
        width = clamp(width, 0, null, self.getWidth())
        height = clamp(height, 0, null, self.getHeight())
        var dashes = Math.round(width * height / 1.5 / workbench.getZoom())
        _boundsBox
            .size(width, height)
            //.rotate(-self.getRotation())
            .center(self.getX(), self.getY())
            .stroke({width: workbench.getZoom() * 0.25, 'dasharray': width * height / dashes})

        _rotateHandle
            .move(self.getX() - width / 2 - 5, self.getY() - height / 2 - 5)
            .rotate(-self.getRotation())

        _moveHandle
            .move(self.getX() + width / 2 - 2, self.getY() - height / 2 - 8)
            .rotate(-self.getRotation())
    }
    ...
    this.getWidth = function() {
        return round(_asset.node.getBoundingClientRect().width / workbench.getDPMM() * workbench.getZoom(), places)
    }
    this.getHeight = function() {
        return round(_asset.node.getBoundingClientRect().height / workbench.getDPMM() * workbench.getZoom(), places)
    }
    ...
    this.fixZoom = function() {
        setBounds(this, this.getWidth(), this.getHeight())
    }
